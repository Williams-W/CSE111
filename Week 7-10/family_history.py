...
def print_death_age(people_dict):
    ...
    Return: nothing
    """
    for deaths in people_dict:
        name = people_dict[deaths][NAME_INDEX]
        death = people_dict[deaths][DEATH_YEAR_INDEX] - people_dict[deaths][BIRTH_YEAR_INDEX]
        print(name, death)
...
def count_genders(people_dict):
    ...
    M = 0
    F = 0
    for sex in people_dict:
        gender = people_dict[sex][GENDER_INDEX]
        if gender == "M":
            M += 1
        else:
            F += 1
...
def print_marriages(marriages_dict, people_dict):
    ...
    #The name and age in the wedding year of the husband
        groom = marriages_dict[x][HUSBAND_KEY_INDEX]
        man = people_dict[groom][NAME_INDEX]
        age_m = marriages_dict[x][WEDDING_YEAR_INDEX] - people_dict[groom][BIRTH_YEAR_INDEX]

    #The year of the wedding
        year = marriages_dict[x][WEDDING_YEAR_INDEX]

    #The name and age in the wedding year of the wife
        bride = marriages_dict[x][WIFE_KEY_INDEX]
        woman = people_dict[bride][NAME_INDEX]
        age_f = marriages_dict[x][WEDDING_YEAR_INDEX] - people_dict[bride][BIRTH_YEAR_INDEX]
        
        print(f"{man}, age {age_m}, married {woman}, age {age_f}, in the year {year}")
