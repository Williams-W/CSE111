def main():
    ...
    for h2o in periodic_table_dict:
        print(periodic_table_dict[h2o][name_index], periodic_table_dict[h2o][mass_index])

    # Get a chemical formula for a molecule from the user.
    formula = input("What is the chemical formula for the molecule: ")

    # Get the mass of a chemical sample in grams from the user.
    mass = float(input("What is the chemical mass of the sample in grams: "))

    # Call the make_periodic_table function and store the periodic table in a variable.
    periodic_table_dict = make_periodic_table()

    # Call the parse_formula function to convert the chemical formula given by the user to a compound list that 
    # stores element symbols and the quantity of atoms of each element in the molecule.
    ...
    # Print chemical name C13H18O2
    known_molecules_dict = suggestions()
    named = get_formula_name(formula, known_molecules_dict)
    print(f'name of formula: {named}')
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for x in symbol_quantity_list:
        element = periodic_table_dict[symbol_quantity_list[y][0]][1] * symbol_quantity_list[y][1]
        total_mass += element
        y += 1
